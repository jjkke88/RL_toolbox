...
from RLToolbox.storage.storage_continous_parallel_image import ParallelStorageImage
...
if __name__ == "__main__":
    ...
    learner = TRPOParallelAgent(learner_env, session=None, baseline=baseline, storage_class=rollouts, distribution=distribution, net_class=net_class, pms=pms, task_q=learner_tasks , result_q=learner_results)
    learner.start()
    ...
    if pms.train_flag is True:
        ...
        from RLToolbox.storage.storage_image import Storage
        session = tf.Session()
        baseline = Baseline()
        storage = None
        pms = PMS_base().pms
